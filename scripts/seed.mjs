...
const sabina = await db.collection('properties').findOne({ slug: 'sabina-yenagoa' })
const usersColl = db.collection('users')
const ada = await usersColl.findOne({ email: 'user@olivhomes.ng' })
if (sabina && ada) {
  const reviews = [
    { propertyId: sabina._id.toString(), userId: ada._id.toString(), authorName: ada.name, rating: 5, text: 'Quiet, secure and very close to the city centre. Power and water are reliable — a great first home in Yenagoa.', status: 'PUBLISHED', createdAt: now, updatedAt: now },
    { propertyId: sabina._id.toString(), userId: agent?._id?.toString() ?? 'seed-agent', authorName: agent?.name ?? 'Renters Network', rating: 4, text: 'Well maintained compound with round-the-clock security. Parking is generous and the compound is family-friendly.', status: 'PUBLISHED', createdAt: now, updatedAt: now },
  ]
  await db.collection('reviews').bulkWrite(reviews.map((review) => ({ updateOne: { filter: { propertyId: review.propertyId, userId: review.userId }, update: { $set: review, $setOnInsert: { createdAt: now } }, upsert: true } })))
}
