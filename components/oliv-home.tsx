...
export function OlivHome() {
  ...
  const { homes } = useOlivState()
  const results = homes.filter((home) => `${home.title} ${home.location.city}`.toLowerCase().includes(query.toLowerCase()))
    ...
    <main>
      <section className="mx-auto grid max-w-7xl gap-12 px-5 pb-20 pt-16 lg:grid-cols-[1.05fr_.95fr] lg:items-center lg:px-10 lg:pb-28 lg:pt-24">
        <div>
          ...
          <div className="mt-9 max-w-2xl rounded-2xl border border-border bg-card p-2 shadow-lg"><div className="flex items-center gap-3 px-3 py-3"><Search className="size-5 text-muted-foreground" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={`Search ${OLIV_MARKET.city} homes...`} className="min-w-0 flex-1 bg-transparent text-sm outline-none" /><a href={`/discover${query ? `?q=${encodeURIComponent(query)}` : ''}`} className="rounded-xl bg-accent px-4 py-2.5 text-sm font-semibold text-accent-foreground">Search</a></div></div>
          <div className="mt-5 flex flex-wrap gap-2 text-xs text-muted-foreground"><a href="/discover?city=Amassoma" className="rounded-full bg-muted px-3 py-1.5">Amassoma</a><a href="/discover?city=Amassoma" className="rounded-full bg-muted px-3 py-1.5">Bayelsa</a></div>
        </div>
      ...
      <section className="border-y border-border bg-muted/40"><div className="mx-auto max-w-7xl px-5 py-16 lg:px-10"><div className="flex items-end justify-between gap-6"><div><p className="text-sm font-semibold uppercase tracking-widest text-accent-foreground">Made for {OLIV_MARKET.city}</p><h2 className="mt-3 font-serif text-4xl tracking-tight sm:text-5xl">Places worth coming home to</h2></div><a href="/discover" className="hidden items-center gap-2 text-sm font-semibold sm:flex">View all homes <ArrowRight className="size-4" /></a></div><div className="mt-10 grid gap-6 lg:grid-cols-3">{results.slice(0, 3).map((home) => <article key={home._id} className="overflow-hidden rounded-2xl border border-border bg-card"><a href={`/listing/${home._id}`}><img src={homeImage(home)} alt={home.title} className="aspect-[4/3] w-full object-cover" /></a><div className="p-5"><h3 className="font-serif text-xl">{home.title}</h3><p className="mt-1 text-sm text-muted-foreground">{home.location.city}, {home.location.state}</p></div></article>)}</div></div></section>
    </main>
  </div>
}
